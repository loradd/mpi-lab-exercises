...
int main (int argc, char *argv[])
{
  ...
  darts_process = DARTS/size + (rank == MASTER ? DARTS % size : 0);

  avepi = 0;
  for (i = 0; i < ROUNDS; i++) {
    homepi = dboard(darts_process);
    MPI_Reduce(&homepi, &pi, 1, MPI_DOUBLE, MPI_SUM, MASTER, MPI_COMM_WORLD);
    if (rank == MASTER) {
      /* Master calculates the average value of pi over all iterations */
      avepi = ((avepi * i) + (pi/(double)size))/(i + 1); 
